...
def main():
    ...
    net = invmodel(spec_dim=spec_dim, layer_dim=layer_dim, hidden_dims=[1024,512,256,128])
    net = net.to(device)
...
def train(net, optimizer,awl, batch=10):#why to use tm.loss_function
    net.train()
    train_loss = 0
    ...
    for idx in range(batch):
        ...
        #loss = invmodel.loss_function(preds,specs)
        # add loss 2
        ...
        print('training mse for idx {} is {}'.format(idx, loss_sum))
    ...
    tm.cal_trans(invdes[0].detach().cpu(), plot=True)
    #target = target.cpu()
    #err_opt = tm.loss_function(opt)
    #err_pre = tm.loss_function1(pre)
    #err_pre = invmodel.loss_function(pre,target)
    #plt.figure(dpi=300)
...
if __name__ == '__main__':
    ...
    mat_lst = ['MgF2Notch532'] + ['metafilmNotch532']+['metafilmNotch532']+['Ag']+['TiO2']
